<template lang="pug">
  #Home
    ...
    v-layout(style="position: absolute top: 8px right: 0 left: 0", justify-space-between)
    ...
    v-layout(column, style="position: absolute bottom: 32px right: 0")
      v-btn(fab)
        v-avatar
          v-img(src="https://picsum.photos/200")
      v-badge.mt-2(overlap, top, left)
        template(#badge) 
          strong 3
        v-btn(fab, color="primary", dark, @click="opened = 'chat'")
          v-icon far fa-comment-dots
      v-btn.mt-2(fab, @click="opened = 'editor'")
    ...
    v-dialog(:value="opened", fullscreen, hide-overlay, transition="dialog-bottom-transition")
      v-card
        contacts(v-if="opened == 'chat'", @back="opened = ''")
        editor(v-else-if="opened == 'editor'", @back="opened = ''")
        search(v-else-if="opened == 'search'", @back="opened = ''", @updated="updateCentre", :centre="userPosition")
