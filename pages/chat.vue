  ...
  #contacts(style="height: 100%")
    div(v-if="!chatSelected")
      ...
      v-list
        ...
        v-list-item(v-for="(chat, i) in chats", :key="i", @click="chatSelected = chat")
          v-list-item-avatar
            v-img(:src="chat.user.profilePicture")
          v-list-item-content
            v-list-item-title.font-weight-bold {{ chat.user.username }}
          v-list-item-action
            v-chip(v-if="chat.unread != 0", color="primary") {{ chat.unread }}
    chat(v-else, :chat="chatSelected", style="height: 100%")
</template>
...
export default {
  ...
  mounted() {
    // this.total = this.chats.reduce((a, b) => a + (b['unread'] || 0), 0)
    this.total = 0
  },
}
</script>
